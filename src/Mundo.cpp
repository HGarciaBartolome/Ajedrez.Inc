...
void Mundo::Mouse(int button, int state, int x, int y)
{
	if ((button == GLUT_LEFT_BUTTON) && (state == GLUT_DOWN))
	{
		...
		// Se tiene en cuenta que no haya casilla seleccionada O que el movimiento NO sea válido O que haya ya una pieza del mismo color
		if ((!tablero.hayCasillaSeleccionada() || !tablero.getCasillaSeleccionada().getPieza()->movValido(v.x, v.y) || tablero.hayPiezaDelMismoColor(v)))
